...
def check_integer(number):
    ...
    return abs(number - int(number)) < 1e-3 or abs(number - math.ceil(number)) < 1e-3


def solve_machine(machine_dict):
    lhs = np.array([[machine_dict["A"][0], machine_dict["B"][0]], [machine_dict["A"][1], machine_dict["B"][1]]])
    rhs = np.array([10000000000000 + machine_dict["Prize"][0], 10000000000000 + machine_dict["Prize"][1]])
    result = np.linalg.solve(lhs, rhs)
    if check_integer(result[0])  and check_integer(result[1]):
